/**
 * Modified January 27 2008 by ivucica
 * Igor surely won't talk to me, ever, since I used this
 * specifically to avoid GMP under MSVC and elsewhere.
 * So I made small mods to his code to make it MSVC-compatible
 * and new-G++ compatible.
 * Original:
...
BigInt BigInt::operator*( long double n )
{
    ...
    char *afterDot;
    afterDot = (char*)malloc(DIGS_AFTER_DOT + 1);
    memset( afterDot, 0, DIGS_AFTER_DOT+1);
