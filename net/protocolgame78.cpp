...
ProtocolGame78::ProtocolGame78(const std::string& accountname, const std::string& password, const std::string& name, bool isGM) :
ProtocolGame(accountname, password, name, isGM)
{
	m_skipTiles = 0;
}

void ProtocolGame78::checkVersion()
{
    ASSERT(ProtocolConfig::getInstance().getClientVersion() == CLIENT_VERSION_780);
}

ProtocolGame78::~ProtocolGame78()
{
	//
}


bool ProtocolGame78::onRecv(NetworkMessage& msg)
{
	m_currentMsg = &msg;
	m_currentMsgN++;
	while(!msg.eof()){
...
Item* ProtocolGame78::internalGetItem(NetworkMessage& msg, uint32_t itemid)
{
...
bool ProtocolGame78::internalSetOutfit(NetworkMessage& msg, Outfit_t& outfit)
{
